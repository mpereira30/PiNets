...
num_trajectories = 40
layer_size = 64 # number of RNN neurons
num_targets = 2
num_layers = 3
...
rnn_tuple = tuple( [ tf.contrib.rnn.LSTMStateTuple( inner_layers_unstack[idx][0], inner_layers_unstack[idx][1] ) for idx in range(num_layers) ] )
 
# Build the RNN graph:
keep_prob = tf.placeholder(tf.float64, shape=())
def lstm_cell():
	return tf.contrib.rnn.DropoutWrapper( tf.contrib.rnn.BasicLSTMCell(layer_size, state_is_tuple=True), output_keep_prob = keep_prob)
	...
	for epoch_idx in range(num_epochs):
		...
		for bidx in range(len(u_inputs)//batch_size):
			...
			for i in range(0,batch_size):
				u[i,:] = u_packet[i]
				y[i,:,:] = y_packet[i]
				_inner_layers[0,0,i,0:2] = x0_packet[i]  # cell state
				_inner_layers[0,1,i,0:2] = x0_packet[i] # hidden state			
		...
		for bidx in range(len(test_inputs)//batch_size):
			...
			_total_loss = sess.run([total_loss],feed_dict={X_train: u, Y_train: y, inner_layers: _inner_layers, keep_prob: 1.0})
